...
- 当前主线：**备份恢复演练（10 件事第 9 项）**。

## 基线验证（2026-09-05，借出+异常联动后）

| 检查项 | 命令 | 结果 |
|---|---|---|
| Go 测试 | `GOSUMDB=sum.golang.org go test ./...` | 通过（新增借出生命周期事件、遗失联动 2 条测试） |
| 前端构建 | `pnpm build` | 通过 |
| E2E | `pnpm test:e2e` | 38 条全部通过（搜索 8 + 消耗品 1 + 借出联动 1） |

## 最近完成

- 2026-09-05：**借出 + 异常联动（10 件事第 8 项）**。
...
- 当前主线：**备份恢复演练（10 件事第 9 项）**——备份包可在新数据目录恢复。
- 待办：README 阶段更新（第 10 项）。

## 下一步

- S2 剩余工作流按 10 件事顺序推进：备份恢复演练 → README 阶段更新。
