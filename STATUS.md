...
- **S3 家庭与运维能力加固（进行中）**：隐私隔离核心已上线；多用户 Owner/Member 边界已验收；
  设置页 E2E 已补齐。

## 基线验证（2026-09-08，设置页 E2E 后）

| 检查项 | 命令 | 结果 |
|---|---|---|
| Go 测试 | `GOSUMDB=sum.golang.org go test ./...` | 通过 |
| 前端构建 | `pnpm build` | 通过 |
| E2E | `pnpm test:e2e` | 41 条全部通过（新增设置页 3 条） |
...
- **S3 家庭与运维能力加固（进行中）**：
  - 隐私隔离核心已上线（读路径 + 借出边界 + 跨账号测试）。
  - 多用户 Owner/Member 边界已验收（Owner-only 403、删除约束、偏好自治）。
  - 设置页 E2E 已补齐（偏好/实例配置自动保存、成员管理）。
  - 下一步：**通知真实 endpoint 验证**（Webhook/Ntfy 真实发出、失败可见）→
    安全检查清单 → 隐私剩余（分类使用计数等聚合路径）。

## 下一步

- 按 S3 顺序推进：通知真实 endpoint 验证 → 安全检查清单 → 隐私剩余聚合路径收尾。
