...
- **S3 家庭与运维能力加固（进行中）**：隐私隔离核心已上线（第一优先高风险项）。

## 基线验证（2026-09-08，S3 隐私隔离核心后）

| 检查项 | 命令 | 结果 |
|---|---|---|
| Go 测试 | `GOSUMDB=sum.golang.org go test ./...` | 通过（新增 7 条跨账号隐私测试） |
| 前端构建 | `pnpm build` | 通过 |
| E2E | `pnpm test:e2e` | 38 条全部通过（隐私过滤对 demo 全公开数据无影响） |
...
- **S3 家庭与运维能力加固（进行中）**：
  ...
  - 下一步：多用户 Owner/Member 边界验收（Owner-only 路由、成员不可改实例配置）→
    设置页 E2E → 通知真实 endpoint 验证 → 安全检查清单。

## 下一步

- 按 S3 顺序推进：多用户边界验收 → 隐私剩余（分类使用计数等聚合路径）→ 设置页 E2E →
  通知真实 endpoint 验证。
