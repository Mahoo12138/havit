...
- **S3 家庭与运维能力加固（进行中）**：隐私隔离核心、多用户边界、设置页 E2E、
  通知真实 endpoint 验证均已完成。

## 基线验证（2026-09-08，通知 endpoint 验证后）

| 检查项 | 命令 | 结果 |
|---|---|---|
| Go 测试 | `GOSUMDB=sum.golang.org go test ./...` | 通过（新增通知失败重试、Ntfy 格式、失败可见性 3 条测试） |
| 前端构建 | `pnpm build` | 通过 |
...
- **S3 家庭与运维能力加固（进行中）**：
  - 隐私隔离核心、多用户边界、设置页 E2E、通知真实 endpoint 验证已完成。
  - 下一步：**安全检查清单**（PAT/JWT/密码/导出文件最小泄漏面）→
    隐私剩余（分类使用计数等聚合路径）→ S2.1 真机验收。

## 下一步

- 按 S3 顺序推进：安全检查清单 → 隐私剩余聚合路径收尾。
